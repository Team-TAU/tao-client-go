...
func TestClient_ModifyChannelInformationReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, at least one parameter must be provided of gameID, language, title, and delay"})
	require.False(t, changed)
}
...
func TestClient_UpdateCustomRewardReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, change can't be nil"})
	require.Nil(t, reward)
}
...
func TestClient_UpdateRedemptionStatusReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, redemptionIDs can't be empty or nil"})
	require.Nil(t, redemptions)
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request,  maximum of 50 redemptionIDs, but you supplied 51"})
	require.Nil(t, redemptions)

	redemptions, err = client.UpdateRedemptionStatus("123", "456", make([]string, 25), "foo")
	require.ErrorIs(t, err, BadRequestError{"invalid request,  status can only be one of FULFILLED or CANCELED"})
	require.Nil(t, redemptions)
}
...
func TestClient_UpdateChannelStreamScheduleReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, if vacationEnabled, vacationStartTime must be specified"})
	require.False(t, changed)

	changed, err = client.UpdateChannelStreamSchedule("123", &vacationEnabled, &now, nil, nil)
	require.ErrorIs(t, err, BadRequestError{"invalid request, if vacationEnabled, vacationEndTime must be specified"})
	require.False(t, changed)

	changed, err = client.UpdateChannelStreamSchedule("123", &vacationEnabled, &now, &now, nil)
	require.ErrorIs(t, err, BadRequestError{"invalid request, if vacationEnabled, timezone must be specified"})
	require.False(t, changed)
}
...
func TestClient_UpdateChannelStreamScheduleSegmentReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, segment id can't be blank"})
	require.Nil(t, segment)
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, update can't be nil"})
	require.Nil(t, segment)
}
...
func TestClient_EndPollReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, poll id can't be blank"})
	require.Nil(t, poll)
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, status must either be TERMINATED or ARCHIVED"})
	require.Nil(t, poll)
...
func TestClient_EndPredictionReturnsError(t *testing.T) {
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, prediction id can't be blank"})
	require.Nil(t, prediction)
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, status must either be RESOLVED, CANCELED, or LOCKED"})
	require.Nil(t, prediction)
	...
	require.ErrorIs(t, err, BadRequestError{"invalid request, if status RESOLVED, winning outcome must be set"})
	require.Nil(t, prediction)
}
